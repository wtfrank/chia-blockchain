...
def create_pooling_inner_puzzle(
    ...
    pool_reward_prefix = bytes32(genesis_challenge[:16] + bytes([0] * 16))
    p2_singleton_puzzle_hash: bytes32 = launcher_id_to_p2_puzzle_hash(launcher_id)
    return POOL_MEMBER_MOD.curry(
        target_puzzle_hash, p2_singleton_puzzle_hash(launcher_id), bytes(owner_pubkey), pool_reward_prefix, pool_waiting_room_inner_hash
    )
...
def create_escape_spend(
    ...
    if is_pool_member_inner_puzzle(inner_puzzle):
        ...
        # XXX should this be pool_info.current or pool_info.target ???
        destination_inner: Program = pool_state_to_inner_puzzle(pool_info.current, pool_info.launcher_id, genesis_challenge)
